...
> from PyNekos import nekosapi
> 
> nyan = nekosapi.Neko(token="xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx", username="myuser", password="iwillnotshowyouthis")
...
## Upload image
Using the **upload_image(image, image_type, tags, image_path, nsfw, artist)** function. The function can upload images from local path, from a url or from a danbooru post. Just need to select the correct one by the **image_type** parameter.
...
The **image_type** parameter is type `string` and is the type of upload. Avaible: *url, local, danbooru* - Required.
